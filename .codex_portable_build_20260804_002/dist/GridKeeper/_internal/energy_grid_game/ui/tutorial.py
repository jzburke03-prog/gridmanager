...
class TutorialManager:
    ...
    def handle_event(self, event, audio=None) -> bool:
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            ...
            if self.state == DialogueState.WAITING_FOR_GAME_ACTION:
                ...
                action_targets = [
                    self._regions.get(key) for key in self.step.get("action_regions", ())
                ]
                ...
                if (target is None or target.collidepoint(event.pos)
                        or any(rect and rect.collidepoint(event.pos)
                               for rect in action_targets)):
    ...
    def draw(self, surface):
        ...
        blocked = [self._regions.get(k) for k in ("chart", "readout",
                                                  "speed_control")]
        blocked.extend(self._regions.get(k) for k in self.step.get("action_regions", ()))
