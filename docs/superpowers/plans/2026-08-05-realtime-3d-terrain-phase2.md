...
- Road role → shape/yaw mapping (CORRECTED post-Phase-2-review, verified against the baked mesh `.npz` bounding boxes and `road_network.py`'s `assign_roles` neighbor semantics — see `terrain3d.py`'s `ROAD_ROLE_TO_SHAPE_YAW` comment for the derivation): `straight_ne`→(straight, 0°), `straight_nw`→(straight, 90°), `corner_ne`→(corner, 0°), `corner_nw`→(corner, 180°), `tee_ne`→(tee, 180°), `tee_nw`→(tee, 270°), `cross`→(cross, 0°). The original mapping below this line's earlier version had `corner_nw` and `tee_ne` wrong (both at 90°/0° respectively instead of 180°), and a subsequent review pass got `tee_nw` wrong too (90° instead of 270°, from a transposed rotation-matrix formula) — do not re-derive from that stale version.
